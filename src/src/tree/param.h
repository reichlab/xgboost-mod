...
//    utils::Printf("max_delta_step == %lf, reg_alpha = %lf, reg_lambda = %lf\n", max_delta_step, reg_alpha, reg_lambda);
    if (max_delta_step == 0.0f) {
//      utils::Printf("max delta step 0\n");
//      if (reg_alpha == 0.0f) {
//        utils::Printf("sum_grad = %lf, sum_hess = %lf, reg_lambda = %lf, gain = %lf\n", sum_grad, sum_hess, reg_lambda, Sqr(sum_grad) / (sum_hess + reg_lambda));
//        return Sqr(sum_grad) / (sum_hess + reg_lambda);
//      } else {
//        return Sqr(ThresholdL1(sum_grad, reg_alpha)) / (sum_hess + reg_lambda);
//      }
      // borrow code from next block down, which doesn't make assumptions about
      // the objective being pos. def. or the particular form of w used.
      double w = CalcWeight(sum_grad, sum_hess);
      double ret = sum_grad * w + 0.5 * (sum_hess + reg_lambda) * Sqr(w);
//      utils::Printf("sum_grad = %lf, sum_hess = %lf, w = %lf, ret = %lf\n", sum_grad, sum_hess, w, ret);
      if (reg_alpha == 0.0f) {
        return - 2.0 * ret;
      } else {
...
//    utils::Printf("a grad = %lf, b grad = %lf, final grad = %lf\n", a.sum_grad, b.sum_grad, a.sum_grad - b.sum_grad);
//    utils::Printf("a hess = %lf, b hess = %lf, final hess = %lf\n", a.sum_hess, b.sum_hess, a.sum_hess - b.sum_hess);
...
//    utils::Printf("grad = %lf, sum_grad = %lf\n", grad, sum_grad);
  }
};
