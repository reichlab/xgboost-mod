...
//      utils::Printf("ndata = %i, positions = ", ndata);
      #pragma omp parallel for schedule(static)
      for (bst_omp_uint i = 0; i < ndata; ++i) {
        const bst_uint ridx = rowset[i];
        const int tid = omp_get_thread_num();
//        utils::Printf("%i-%i-%i  ", tid, ridx, position[ridx]);
        if (position[ridx] < 0) continue;
        stemp[tid][position[ridx]].stats.Add(gpair, info, ridx);
      }
      // sum the per thread statistics together
//      utils::Printf("\nline 216, InitNewNode, qexpand size = %i, stemp size = %i\n", qexpand.size(), stemp.size());
...
//        utils::Printf("root gain = %lf\n", snode[nid].root_gain);
    ...
    inline void ParallelFindSplit(const ColBatch::Inst &col,
                                  ...
                                  const std::vector<bst_gpair> &gpair,
                                  const BoosterInfo &info) {
//      utils::Printf("in parallel find split\n");
...
//      utils::Printf("in parallel find split, need forward = %s, need backward = %s\n", need_forward ? "true" : "false", need_backward ? "true" : "false");
...
//              utils::Printf("loss_chg 3 = %lf\n", e.stats.CalcGain(param) +
...
//            utils::Printf("c.sum_hess = %lf, tmp.sum_hess = %lf, min child weight = %lf\n", c.sum_hess, tmp.sum_hess, param.min_child_weight);
            if (c.sum_hess >= param.min_child_weight &&
                tmp.sum_hess >= param.min_child_weight) {
//              utils::Printf("loss_chg 1 = %lf\n", tmp.CalcGain(param) +
//                c.CalcGain(param) - snode[nid].root_gain);
...
//                utils::Printf("c.sum_hess = %lf, cright.sum_hess = %lf, min child weight = %lf\n", c.sum_hess, cright.sum_hess, param.min_child_weight);
                if (c.sum_hess >= param.min_child_weight &&
                    cright.sum_hess >= param.min_child_weight) {
//                  utils::Printf("loss_chg = %lf\n", cright.CalcGain(param) +
//                    c.CalcGain(param) -
//                    snode[nid].root_gain);
...
//      utils::Printf("fvalue = %lf\n\n", fvalue);
      ...
      if (e.stats.Empty()) {
        ...
        // try to find a split
//        utils::Printf("fvalue = %lf\n", fvalue);
//        utils::Printf("e.stats.sum_grad = %lf, e.stats.sum_hess = %lf\n", e.stats.sum_grad, e.stats.sum_hess);
        if (std::abs(fvalue - e.last_fvalue) > rt_2eps &&
            e.stats.sum_hess >= param.min_child_weight) {
          c.SetSubstract(snode[nid].stats, e.stats);
          if (c.sum_hess >= param.min_child_weight) {
//            utils::Printf("\nfvalue = %lf, cut pt = %lf\n", fvalue, (fvalue + e.last_fvalue) * 0.5f);
//            utils::Printf("loss_chg 10 = %lf, e.gain = %lf, c.gain = %lf, root_gain = %lf\n",
//              e.stats.CalcGain(param) +
//                c.CalcGain(param) - snode[nid].root_gain,
//              e.stats.CalcGain(param),
//              c.CalcGain(param),
...
//      utils::Printf("updater_colmaker-inl.hpp line 602 qexpand.size = %i, qexpand[0] = %i\n", qexpand.size(), qexpand[0]);
      for (size_t i = 0; i < qexpand.size(); ++i) {
        const int nid = qexpand[i];
        NodeEntry &e = snode[nid];
        // now we know the solution in snode[nid], set split
//        utils::Printf("updater_colmaker-inl.hpp line 619 best loss chg = %lf, rt_eps = %lf, split value = %lf\n", e.best.loss_chg, rt_eps, e.best.split_value);
